#!/usr/bin/env python3
"""Copy cached Japanese translations into data/papers.jsonl metadata."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Sync title_ja and abstract_ja from translation cache to papers.jsonl."
    )
...
def main() -> int:
    ...
    translations = json.loads(args.cache.read_text(encoding="utf-8"))
    ...
    for paper in papers:
        ...
        translated = translations.get(content_id)
        if not translated:
            missing += 1
            continue
        title_ja = clean_text(translated.get("title_ja"))
        abstract_ja = clean_text(translated.get("abstract_ja"))
        if not title_ja and not abstract_ja:
            empty += 1
            continue
        metadata = paper.setdefault("metadata", {})
        if title_ja:
            metadata["title_ja"] = title_ja
        if abstract_ja:
            metadata["abstract_ja"] = abstract_ja
        updated += 1
